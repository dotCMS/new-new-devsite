    ...
    <li className="list-none">
      <button
        ...
        className={cn(
          ...
          // Inset on the left: active white uses this for space before the pink pill; same on idle so rows don’t jump
          "py-1.5 pl-2.5",
          // Span the section’s horizontal padding on the right; hover and active use the same width
          "-mr-3 pr-3",
...
export function OverviewSectionNav({
  className,
  isMobile = false,
}: OverviewSectionNavProps) {
  const [activeId, setActiveId] = React.useState<string | null>("what-is-dotcms");
  ...
  return (
    ...
    >
      <div
        className={cn(
          "min-h-0",
          isMobile && scrollClass,
          "pt-0"
        )}
      >
        <DocsSidebarFilter />
        <div className="pt-4">
          {overviewNavSections.map((section, sectionIndex) => (
            <div
              key={section.id}
              className={cn(
                paddingX,
