...
import {
  ...
  type ReferenceNavLink,
} from "./referenceNavData";

const SIDEBAR_BG = "bg-[#F6F6F7] dark:bg-muted/25";

type NavRowProps = {
  item: ReferenceNavLink;
  isActive: boolean;
...
type ReferenceSectionNavProps = {
  className?: string;
  isMobile?: boolean;
};

export function ReferenceSectionNav({
  className,
  isMobile = false,
}: ReferenceSectionNavProps) {
  const blocks = referenceNavBlocks;
  const [activeId, setActiveId] = React.useState<string | null>(() =>
    firstReferenceActiveId(blocks) || null
  );
